...
<%@ page import="com.google.appengine.api.datastore.*, com.google.appengine.api.datastore.Query.*, java.util.Iterator
...
<h1>Welcome</h1>
	...
	while(listOfStudent.hasNext()) { 
		...
		  <label for="absent">absent
		 <input type="hidden" id="id" name="id" value=<%= e.getKey().getName().toString()%>>
		<input type="hidden" id="name" name="name" value=<%= e.getProperty("name").toString()%>>
		<input type="hidden" id="email" name="email" value=<%= e.getProperty("email").toString()%>>
...
<form action = "DisplayTheStudentAttendance" method = "post">
<label
			for="date"><b>Date:</b></label> <input type="date" id="date"
			name="date" required>
<input type = "submit"> 
